...
optional<pair<string, string>> read_socket_frame(int client_fd) {
    ...
    if (bytes_received != HEADER_SIZE) {
        if (bytes_received == 0) {
            ...
            std::cout << "fail to read header of type" << std::endl;
        }
        return nullopt;
    }
    ...
    if (bytes_received != topic_length) {
        std::cout << "fail to read complete topic" << std::endl;
        return nullopt;
    }
    ...
    if (bytes_received != HEADER_SIZE) {
        if (bytes_received == 0) {
            ...
            std::cout << "fail to read header of length" << std::endl;
        }
        return nullopt;
    }
    ...
    if (message_length > MAX_MESSAGE_SIZE) {
        std::cout << "message from client is too large, which length is " << message_length << std::endl;
        return nullopt;
    }
    ...
    if (bytes_received != message_length) {
        std::cout << "fail to read complete message from client" << std::endl;
        return nullopt;
    }
...
int main(int argc, char const *argv[]) {
    ...
    thread subcribe_thread([&topics, server_ip, server_port] {
        ...
        while (true) {
            ...
            std::cout << "subscribe message body: " << body << std::endl;

            if (!send_socket_data(client_fd, "subscribe", body)) {
                std::cout << "fail to send subscribe message" << std::endl;
                close(client_fd);
                continue;
            }
            ...
            if (!subscribe_resp.has_value()) {
                std::cout << "fail to read subscribe response" << std::endl;
                close(client_fd);
                continue;
            }
            ...
            if (subscribe_resp.value().second.find("ok") == std::string::npos) {
                // fail
                std::cout << "fail to subscribe: " << subscribe_resp.value().second << std::endl;
                close(client_fd);
                continue;
            } else {
            ...
            thread ping_thread([client_fd, &socket_disconnected] {
                ...
                while (true) {
                    ...
                    if (!send_result) {
                        std::cout << "fail to send ping" << std::endl;
                        socket_disconnected = true;
                        break;
                    }
                }
                std::cout << "ping thread exit" << std::endl;
            });
            ping_thread.detach();
            ...
            while (true) {
                ...
                if (topic_name == "pong") {
                    std::cout << "receive pong message: " << message_body << std::endl;
                    continue;
                } else {
